...
import {type Steps, type Tokens} from './types';
...
export const tokenify = (steps: Steps, tokens: Tokens) =>
  fromEntries(
    entries(steps).map(<Key extends number>([key, {canGo: canGoImpl, ...step}]: [Key, Steps[keyof Steps]]) => {
...
export const checkSteps = (steps: Steps) => {
  ...
  const requiredKeys = Array.from({length: range.end}, (_, index) => index + range.start);
  const canWork = requiredKeys.every(requiredKey => keys.includes(requiredKey));
